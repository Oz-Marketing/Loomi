...
### Campaigns
A multi-channel container orchestrating the above, and a client's Studio home. Three ways a campaign comes to exist, labeled by source on the card: **AI** — the user describes a campaign in plain language, Claude returns a structured plan (audience suggestion, email/SMS/landing-page/form specs, a flow when the goal is an ongoing sequence, clarifying questions), and on generate the assets stream in (SSE) as editable drafts; **Manual** — the user adds each piece by hand; **Automated** — the OEM offer run builds the account's manufacturer offers into ad designs (one per permitted template) plus the companion offer email, one campaign per account per month, and the account's users pick the design to use on the campaign. The builder never auto-sends. Clients see only Automated campaigns.
