...
- **Pacer** — live spend vs. plan (recommended daily budget, projected spend, account-wide pace %), synced from the ad platforms. On the **Meta** card the "do-nothing" projected spend (and the live preview while editing the daily budget) counts **today as a full budget-day** rather than prorating it: `projected = cumulative − today_spend + daily_budget × whole_days_remaining_incl_today` (`doNothingProjected` in `pacer-calc.ts`). `today_spend` is synced from Meta (a time_range {today,today} insights pull → `pacerTodaySpend`) and subtracted back out to avoid double-counting today's already-spent portion. Google keeps its own §5 projection; the shared `buildPacerCalc.projected` (used by Over/Under, reconciliation, alerts) is unchanged.
