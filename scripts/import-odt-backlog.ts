...
import { resolveOrgAccounts, ORG_OVERRIDES } from './_odt-org-map';
...
async function main() {
  ...
  // ── org → account mapping (shared resolver) ──
  const accounts = await prisma.account.findMany({ select: { key: true, dealer: true } });
  const mapping = resolveOrgAccounts(manifest.organizations, accounts);
