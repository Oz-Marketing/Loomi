...
import { FieldPrefsModal } from '@/components/ad-generator/field-prefs-modal';
import { applyFieldPrefs } from '@/lib/ad-generator/field-prefs';
import { boundFieldKeys } from '@/lib/ad-generator/doc-types';
...
export default function AdGeneratorPage() {
  ...
  /** Produced by the automation rather than by a person — see the banner below. */
  const [autoGenerated, setAutoGenerated] = useState(false);
  /** Field keys this sub-account hides on this template's form. */
  const [hiddenFields, setHiddenFields] = useState<string[]>([]);
  const [fieldPrefsOpen, setFieldPrefsOpen] = useState(false);
  /** Manufacturer rules this ad currently breaks — blocks export, like a missing field. */
  const [coopBlocking, setCoopBlocking] = useState(0);
  ...
  useEffect(() => {
    ...
    fetch(`/api/ad-generator/creatives/${creativeId}`)
      ...
      .then((d: {
        ...
        setAutoGenerated(!!c.autoGenerated);
  ...
  const disclaimerPreviewStyle = useMemo<CSSProperties | undefined>(() => {
    ...
    return {
      ...
      // Clamped at BOTH ends. The floor keeps a tiny on-canvas size readable;
      // the ceiling was missing, so a template whose disclaimer sits large on a
      // 1080px canvas rendered at that size in the form and swamped it. Family,
      // weight, colour and alignment still carry the design — size is the one
      // property that has to serve the form instead of the ad.
      fontSize: Math.min(14, Math.max(12, Math.round(box?.fontSize ?? 13))),
    };
  ...
  // One reason string for every export control, so they can't disagree about why
  // the button is off.
  const exportBlockedReason =
    missing.length > 0
      ? 'Fill the required fields before exporting'
      : coopBlocking > 0
        ? 'This ad breaks a manufacturer rule — see Manufacturer compliance'
        : undefined;
  ...
  // This sub-account's hidden-field preference for this template. Re-fetched when
  // either changes; a failure leaves every field showing, which is the safe way
  // for this to break.
  useEffect(() => {
    ...
    fetch(
      ...
      .then((d: { hiddenFields?: string[] }) => {
        if (!cancelled) setHiddenFields(d.hiddenFields ?? []);
      })
      .catch(() => {
        if (!cancelled) setHiddenFields([]);
      });
    return () => {
      cancelled = true;
    };
  ...
  const groups = useMemo(() => {
    ...
    // The sub-account's own trim of the form. `applyFieldPrefs` re-sanitizes, so
    // a preference stored before a field became required cannot hide it.
  ...
  // Keys this design actually draws. The system-field schema is fixed, so every
  // doc carries `vehicleImageUrl` whether or not it has an image element — which
  // is why a template with no vehicle image was still asking for a paint colour.
  const boundKeys = useMemo(() => (docSnapshot ? boundFieldKeys(docSnapshot) : null), [docSnapshot]);

  const vehicleSlots = useMemo<VehicleSlot[]>(
    () =>
      template.fields
        .filter((f) => /vehicleimageurl/i.test(f.key) && isFieldVisible(f, data))
        // No element bound to it ⇒ the ad has nowhere to put a vehicle image, so
        // the colour picker would be asking a question with no effect.
        .filter((f) => !boundKeys || boundKeys.has(f.key))
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-8">
      ...
      {/* An ad the automation built from a manufacturer offer. The form is now
          for custom ads, so nothing else on this page explains where these
          numbers came from — and someone hand-editing a payment here is editing
          a figure the feed will reassert on the next run. Editable on purpose: a
          manager still needs to be able to correct one without regenerating. */}
      {autoGenerated && (
        <div className="mb-6 flex items-start gap-3 rounded-xl border border-amber-500/30 bg-amber-500/10 px-4 py-3">
          ...
          <div className="text-xs text-[var(--foreground)]">
            <span className="font-semibold">Generated from a manufacturer offer.</span>{' '}
            The vehicle, offer numbers and disclaimer came from the OEM feed, and the disclaimer is the
            manufacturer&rsquo;s own wording. You can still edit anything here, but a change to the offer
            figures will be overwritten if this ad regenerates.
          </div>
        </div>
      )}
        ...
        <div className="space-y-6">
          ...
          {accountKey && (
            <div className="flex justify-end">
              <button
                type="button"
                onClick={() => setFieldPrefsOpen(true)}
                className="inline-flex items-center gap-1.5 text-[11px] font-medium text-[var(--muted-foreground)] transition-colors hover:text-[var(--foreground)]"
              >
        ...
        <div className="lg:sticky lg:top-6 lg:self-start">
          <div className="glass-card rounded-2xl border border-[var(--border)] p-5">
            ...
            <div className="mt-4 space-y-2">
              {/* A manufacturer rule that's broken blocks export exactly like a
                  missing required field — both mean the ad can't ship as it is,
                  and the panel on the left says which rule and cites the section. */}
              {coopBlocking > 0 && (
                <div className="flex items-start gap-2 rounded-lg border border-rose-500/40 bg-rose-500/10 px-3 py-2 text-[11px] leading-snug text-rose-600 dark:text-rose-400">
                  <ExclamationTriangleIcon className="mt-0.5 h-3.5 w-3.5 flex-shrink-0" />
                  <span>
                    {coopBlocking} manufacturer {coopBlocking === 1 ? 'rule is' : 'rules are'} broken —
                    see <span className="font-medium">Manufacturer compliance</span> for what to change.
                  </span>
                </div>
              )}
              {missing.length > 0 && (
      ...
      {fieldPrefsOpen && accountKey && (
        <FieldPrefsModal
          accountKey={accountKey}
          templateId={templateId}
          templateName={template.name}
          onClose={() => setFieldPrefsOpen(false)}
          onSaved={setHiddenFields}
        />
      )}
