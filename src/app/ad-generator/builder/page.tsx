...
import {
  BLEED as SHARED_BLEED,
  MIN_FRAC as BLEED_MIN_FRAC,
  groupMoveRange,
} from '@/lib/ad-generator/bleed';
...
// Re-exported from `@/lib/ad-generator/bleed`, which is where the bleed range
// and its rules are defined and tested. Kept as local aliases so the ~200 call
// sites below read unchanged.
const MIN_FRAC = BLEED_MIN_FRAC;

/**
 * How far past each edge an element may be pushed — see
 * `@/lib/ad-generator/bleed`, which owns the range, the rules and their tests.
 *
 * Elements bleed past the artboard (clipped on export) and may be dragged fully
 * off it, at which point they read as detached (`isDetached`).
 *
 * The recurring bug is two paths disagreeing about the range: one allows the
 * overhang, another re-clamps to `[0, 1 - w]` and takes it away. That has now
 * been found four times — single-element drag, group drag, group resize, and
 * arrow-key nudge — which is why the rule lives in one tested place rather than
 * being re-derived at each call site.
 */
const BLEED = SHARED_BLEED;
 ...
 */
// `ContentSource` and `TokenTextArea` moved to `components/token-textarea` so
// the email builder binds data through the same control. See that file.
...
export default function AdBuilderPage() {
    ...
    if (d.kind === 'single') {
      ...
      for (const it of d.items) {
        const nb: DocLayoutBox = {
          ...it.start,
          // `computeBox` above already allowed the bleed; clamping each member to
          // [0, 1] here took it straight back — the same two-clamps-disagreeing
          // bug BLEED was introduced to fix, surviving on the group path.
          x: clamp(rect.x + (it.start.x - d.bounds.left) * scaleX, -BLEED, 1 + BLEED),
          y: clamp(rect.y + (it.start.y - d.bounds.top) * scaleY, -BLEED, 1 + BLEED),
  ...
  function startGroupDrag(e: React.PointerEvent, ids: string[] = selectedIds) {
    ...
    dragRef.current = {
      ...
      // The group's bounding box obeys the same rule one element's box does —
      // see groupMoveRange, which is where that rule is written down and tested.
      ...groupMoveRange({ left, right, top, bottom }),
      targetsX: tx,
      targetsY: ty,
      live: {},
    };
    ...
    function onKey(e: KeyboardEvent) {
      ...
      setDoc((prev) =>
        selectedIds.reduce((acc, id) => {
          const b = acc.layouts[size.id]?.[id];
          if (!b) return acc;
          // Same bleed range the drag uses. Clamping to [0, 1 - w] here meant an
          // element you had just dragged off the edge jumped back inside the
          // moment you nudged it with an arrow key — the two paths disagreeing
          // about where an element may live, which is the bug BLEED exists to
          // settle. Applies to every selected element, so it covers groups.
          const moved = {
            ...b,
            x: clamp(b.x + dx, -b.w - BLEED, 1 + BLEED),
            y: clamp(b.y + dy, -b.h - BLEED, 1 + BLEED),
          };
