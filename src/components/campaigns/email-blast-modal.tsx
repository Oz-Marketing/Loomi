...
export function EmailBlastModal({
  ...
  <p style="margin:0 0 16px;line-height:1.5;">Write the primary blast message here.</p>
  ...
  useEffect(() => {
    ...
    if (!htmlContent.trim()) {
      if (selected.content) {
        ...
        setHtmlContent(
          `<div style="font-family:Arial,sans-serif;padding:24px;"><h2>${selected.label}</h2><p>Start editing this blast template HTML.</p></div>`,
        );
      }
    }
    if (!subject.trim()) {
      setSubject(selected.label.split(' · ')[0] || 'Loomi Blast');
    }
  ...
  async function submitCampaign() {
    ...
    try {
      ...
      const res = await fetch('/api/blasts/email', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          name: campaignName.trim() || `Email Blast ${new Date().toLocaleDateString('en-US')}`,
      ...
      if (!res.ok) {
        throw new Error(typeof data.error === 'string' ? data.error : 'Failed to create email blast');
      }

      if (scheduledIso) {
        const scheduledDate = new Date(scheduledIso);
        setSuccess(`Email blast scheduled for ${scheduledDate.toLocaleString('en-US')}.`);
      } else {
        const sentCount = Number(data?.campaign?.sentCount || 0);
        setSuccess(`Email blast sent to ${sentCount.toLocaleString()} contact${sentCount === 1 ? '' : 's'}.`);
      }
      ...
      setError(err instanceof Error ? err.message : 'Failed to create email blast');
    } finally {
      setSending(false);
    }
  }
        ...
        <div className="p-4 border-b border-[var(--border)] flex items-center justify-between gap-3">
          <div>
            <h3 className="text-base font-semibold">
              {mode === 'both' ? 'Create Email + Text Blast' : 'Create Email Blast'}
            </h3>
            <p className="text-xs text-[var(--muted-foreground)] mt-1">
              {step === 'source'
                ? 'Choose how you want to build this blast email.'
            ...
            <div className="p-5 space-y-4">
              ...
              {mode === 'both' && (
                <p className="text-xs text-[var(--muted-foreground)]">
                  After this email is created, the SMS/MMS composer will open so you can schedule the matching text blast.
                </p>
              )}
