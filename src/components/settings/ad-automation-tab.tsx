 ...
 * rather than a tool.
 *
 * A STUDIO settings tab (Settings → Ad Automation): what it configures is the
 * account's unattended pipeline — watch scope, offer policy, run caps — and the
 * run history and dry run are how you check that config is right, so the whole
 * screen travels together into the sector's rail. The old
 * /ad-generator/automation route redirects here for staff; the dealer-facing
 * template card stays on that route, since clients have no settings rail.
 */
...
export function AdAutomationTab() {
  ...
  // `userRole` is null until the session resolves. Hold a placeholder rather
  // than rendering the inspector against an unknown role.
  if (userRole === null) {
    return (
      <div>
      ...
      >
        ...
        <section className="glass-card h-fit rounded-2xl border border-[var(--border)] p-5">
          ...
          <button
            ...
            className="mt-5 flex w-full items-center justify-center gap-2 rounded-lg bg-[var(--primary)] px-4 py-2.5 text-sm font-medium text-[var(--primary-foreground)] transition-opacity hover:opacity-90 disabled:opacity-50"
          >
