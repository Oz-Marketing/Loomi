 ...
 * global); when none exist it uses the code-defined defaults.
 *
 * A STUDIO settings tab (Settings → Disclaimers), reached from the sector rail
 * rather than the cog on the generator's header — this is legal text the whole
 * sector's ads inherit, not a per-ad tool. The old /ad-generator/templates route
 * redirects here. Admin-only, and the tab itself is admin-gated in the registry;
 * the guard below stays for a deep link that outruns the rail.
 */
...
export function AdDisclaimersTab() {
    ...
    <div>
      <header className="mb-6 flex flex-wrap items-start justify-between gap-3">
        <p className="max-w-3xl text-sm text-[var(--muted-foreground)]">
          Reusable legal text with {'{{slug}}'} tokens, per make + offer type. The generator
          auto-fills the disclaimer from these; numbers fill from the offer.
        </p>
        {!draft && (
          <button
