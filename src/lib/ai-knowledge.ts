...
export async function getAssistantSystemPrompt(
  ...
  sections.push(
    ...
    '- Use a real account logo from the current account context when available. Choose the logo variant that fits the background. Fall back to {{custom_values.logo_url}} only if no account logo URL is available.',
    '- Read and incorporate relevant business details from the account profile when useful: business name, category, address, city/state/postal code, phone numbers, website, email, timezone, and custom values.',
