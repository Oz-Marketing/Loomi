...
PROHIBITED-TERM AND REQUIRED-ITEM LISTS. These documents list banned wording in bulk — thirty terms under one sentence like "the following may not be used in any advertising". Make ONE rule per term. For those, \`quote\` is the term itself (that IS the evidence) and you MUST also give \`context\`: the full sentence or heading, on the same page, that establishes the list forbids or requires things. Both are checked, and both must be on that page. A term with no context is discarded, so do not omit it. Keep \`phrase\` consistent with the term you quoted — a rule about one term supported by a quote of a different one is discarded.
